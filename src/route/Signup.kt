...
fun Route.signup(hashFunction: (String) -> String) {
    post<Signup> {
        ...
        val user = call.sessions.get<AdminUserSession>()?.let { db.user(it.userId) }
        ...
        when {
            ...
            db.user(userId) != null ->
                ...
                call.sessions.set(AdminUserSession(newUser.userId))
    ...
    get<Signup> {
        val db by kodein().instance<PlaygroundRepository>()
        val user = call.sessions.get<AdminUserSession>()?.let {
            db.user(it.userId)
        }
